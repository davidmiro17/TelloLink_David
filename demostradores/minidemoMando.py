...
class MiniRemoteApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Demo Tello Mini Remote")
    ...
    def on_takeoff(self):
        ...
        if isinstance(bat, int) and bat < BAT_MIN_SAFE:
            if not messagebox.askokcancel("Batería baja", f"Batería {bat}%. ¿Despegar igualmente?"):
                return
        try:
            # --- NUEVO: pausar keepalive para no cruzar respuestas ---
            ...
            # reanuda un pelín después (deja respirar al SDK)
            self.root.after(200, self._resume_keepalive)

    def on_land(self):
        # --- NUEVO: debounce de UI para evitar múltiples lands en ráfaga ---
        if getattr(self, "_ui_landing", False):
            return
        self._ui_landing = True
        try:
            # --- NUEVO: pausar keepalive durante la orden ---
            self._pause_keepalive()
            ...
            self.root.after(200, self._resume_keepalive)
            # quita el debounce tras un tiempo prudente
    ...
    def do_move(self, cmd: str):
        ...
        try:
            ...
            # --- NUEVO: pausar keepalive alrededor del comando ---
            self._pause_keepalive()
    ...
    def do_turn(self, dir_: str):
        ...
        try:
            ...
            # --- NUEVO: pausar keepalive alrededor del giro ---
            self._pause_keepalive()
            if dir_ == "cw":
                self.dron.cw(ang)
            else:
    ...
    # --- NUEVO: helpers de pausa/reanudación del keepalive ---
